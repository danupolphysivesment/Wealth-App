...
def simulate_series_paths(mu_m: float, sigma_m: float, months: int = 120,
                          n_sims: int = 2500, seed: int = 42) -> np.ndarray:
    """Monte Carlo growth paths of $1 for a portfolio described by its own
    monthly mean/volatility (used for the constructed fund portfolio)."""
    rng = np.random.default_rng(seed)
    rets = rng.normal(mu_m, sigma_m, size=(n_sims, months))
    paths = np.cumprod(1.0 + rets, axis=1)
    return np.concatenate([np.ones((n_sims, 1)), paths], axis=1)
