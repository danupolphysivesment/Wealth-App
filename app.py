...
@st.cache_data
def get_pulse() -> dict:
    with open(os.path.join(_DATA_DIR, "market_pulse.json")) as f:
        return json.load(f)


@st.cache_data
def get_pulse_trends() -> pd.DataFrame:
    return pd.read_csv(os.path.join(_DATA_DIR, "market_pulse_trends.csv"),
...
with tabs[7]:
    pulse = get_pulse()
    trends = get_pulse_trends()
    topic_of_term = {t["term"]: t for t in pulse["topics"]}

    edu_card("📡", "Search data = the market's mood ring",
             "Google Trends measures what the world is <i>actually</i> worried or "
             "excited about — often before it shows up in prices. Below: the themes "
             "dominating financial search interest right now, what they mean in "
             "plain English, and which part of <i>your</i> plan each one touches. "
             f"Curated by your advisor on "
             f"{pd.Timestamp(pulse['as_of']):%d %B %Y}.")
    ...
    mfig.update_layout(title="Which themes are running hot? Search interest, last 4 "
                             "weeks vs each theme's own 12-month average",
                       xaxis_ticksuffix="%")
    st.plotly_chart(style(mfig, h=340), use_container_width=True)

    st.markdown("### 🔍 Explore a theme")
    sel_title = st.selectbox(
        "Theme", [t["title"] for t in pulse["topics"]],
    ...
    with cch:
        ...
        tfig = go.Figure(go.Scatter(x=s.index, y=s, fill="tozeroy",
                                    ...
                                    name=topic["term"]))
        tfig.update_layout(
            title=f"Google search interest — “{topic['term']}” (12 months, 100 = peak)",
            yaxis_range=[0, 108])
        st.plotly_chart(style(tfig, h=340), use_container_width=True)
    with cdet:
        edu_card(topic["icon"], topic["title"],
                 f"<b>What it is.</b> {topic['what']}<br><br>"
                 f"<b>Why it's everywhere.</b> {topic['why_now']}")

    st.markdown("### 🧭 What each theme means for your portfolio")
        ...
        for col, t in zip(cols, pulse["topics"][row_start:row_start + 3]):
            with col:
                edu_card(t["icon"], t["title"],
                         f"{t['for_you']}<br><br>"
                         f"<i>Sources: {', '.join(t['sources'])} · see "
                         f"{t['related_tab']}</i>")

    st.caption(f"📡 {pulse['intro_note']} Search-interest series come from Google "
               "Trends (each scaled to its own 12-month peak = 100). This is an "
               "advisor-curated snapshot — refresh the data with "
               "`scripts/refresh_market_pulse.py` — and market commentary, not "
               "investment advice.")
