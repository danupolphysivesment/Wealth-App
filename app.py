...
def get_benchmark() -> pd.DataFrame:
    return attribution.load_benchmark()
...
@st.cache_data
def get_pulse(region: str) -> dict:
    suffix = "" if region == "global" else "_th"
    with open(os.path.join(_DATA_DIR, f"market_pulse{suffix}.json")) as f:
        return json.load(f)


@st.cache_data
def get_pulse_trends(region: str) -> pd.DataFrame:
    suffix = "" if region == "global" else "_th"
    return pd.read_csv(os.path.join(_DATA_DIR, f"market_pulse_trends{suffix}.csv"),
...
tabs = st.tabs(["🏠 Overview", "🎯 Allocation Advisor", "🧱 Fund Lab · Core–Satellite",
                "🌪️ Stress Lab", "🔮 Future Paths", "📜 Track Record",
                "🧮 Attribution", "📡 Market Pulse", "🎓 Learn"])
...
# ================================================================== ATTRIBUTION
with tabs[6]:
    edu_card("🧮", "Attribution = the performance autopsy",
             "We compare your portfolio against the classic <b>60/40 autopilot</b> "
             "(60% world equities — MSCI ACWI · 40% global aggregate bonds). The gap "
             "is <b>active return</b>, and it has exactly two sources: "
             "<b>allocation</b> — holding different <i>amounts</i> of each asset "
             "class than the autopilot — and <b>selection</b> — picking funds that "
             "beat (or trail) their own asset class. <b>Beta</b> tells you how much "
             "of the ride is simply the market moving you.")

    which_pf = st.radio("Attribute…",
                        ["Your fund portfolio", f"Recommended ({profile})"],
                        horizontal=True, key="attr_which")
    att_eff = cur["eff"] if which_pf == "Your fund portfolio" else rec["eff"]
    bench = get_benchmark()
    class_rets, class_w = attribution.class_breakdown(fund_rets, att_eff)
    att = attribution.attribute(class_rets, class_w, bench)
    bs = attribution.beta_stats(att["port"], att["bench"])

    port_ann = float(att["port"].mean() * 12)
    bench_ann = float(att["bench"].mean() * 12)
    alloc_ann, sel_ann = att["alloc"].mean() * 12, att["sel"].mean() * 12
    active_ann = port_ann - bench_ann
    st.caption(f"Window: **{att['port'].index[0]:%b %Y} → "
               f"{att['port'].index[-1]:%b %Y}** ({len(att['port'])} months) — "
               "the overlap of your funds' history and the benchmark series.")

    b1, b2, b3, b4, b5, b6 = st.columns(6)
    b1.metric("Portfolio / yr", pct(port_ann))
    b2.metric("60/40 / yr", pct(bench_ann))
    b3.metric("Active return / yr", pct(active_ann),
              delta="ahead" if active_ann >= 0 else "behind",
              delta_color="normal" if active_ann >= 0 else "inverse")
    b4.metric("Beta vs 60/40", f"{bs['beta']:.2f}",
              help="How strongly the portfolio moves with the 60/40. 1.00 = same "
                   "sensitivity; 0.70 = only 70% of the benchmark's swings.")
    b5.metric("Alpha / yr", pct(bs['alpha_ann']),
              help="Return left over after paying for benchmark exposure (beta). "
                   "The 'skill' residual of the regression.")
    b6.metric("Tracking error", pct(bs['tracking_error']),
              delta=f"Info ratio {bs['info_ratio']:.2f}", delta_color="off",
              help="Typical size of the yearly gap vs the 60/40. The information "
                   "ratio is active return earned per unit of that gap.")

    wf_attr = go.Figure(go.Waterfall(
        x=["60/40 benchmark", "Allocation effect", "Selection effect",
           "Your portfolio"],
        measure=["absolute", "relative", "relative", "total"],
        y=[bench_ann * 100, float(alloc_ann.sum()) * 100,
           float(sel_ann.sum()) * 100, 0],
        connector=dict(line=dict(color="#C9CFDA")),
        decreasing=dict(marker=dict(color="#D2483F")),
        increasing=dict(marker=dict(color="#2E8B6B")),
        totals=dict(marker=dict(color="#1C2233")),
        text=[f"{v:+.1f}%" for v in [bench_ann * 100, float(alloc_ann.sum()) * 100,
                                     float(sel_ann.sum()) * 100, port_ann * 100]],
        textposition="outside",
    ))
    wf_attr.update_layout(title="From the 60/40 to your return (% per year)",
                          yaxis_ticksuffix="%")
    st.plotly_chart(style(wf_attr, h=380), use_container_width=True)

    ca1, ca2 = st.columns(2)
    with ca1:
        cls_order = [c for c in attribution.CLASSES if c in att["alloc"].columns]
        eff_fig = go.Figure([
            go.Bar(x=cls_order, y=[alloc_ann.get(c, 0) * 100 for c in cls_order],
                   name="Allocation", marker_color="#2F62F0"),
            go.Bar(x=cls_order, y=[sel_ann.get(c, 0) * 100 for c in cls_order],
                   name="Selection", marker_color="#E09F3E"),
        ])
        eff_fig.update_layout(barmode="group", yaxis_ticksuffix="%",
                              title="Effects by asset class (% per year)")
        st.plotly_chart(style(eff_fig, h=360), use_container_width=True)
        st.caption("Alternatives are an off-benchmark bet, so their whole effect is "
                   "allocation; Multi-Asset funds are judged against the 60/40 "
                   "itself, so theirs is selection.")
    with ca2:
        cum = pd.DataFrame({
            "Allocation": att["alloc"].sum(axis=1).cumsum() * 100,
            "Selection": att["sel"].sum(axis=1).cumsum() * 100,
        })
        cum["Total active"] = cum["Allocation"] + cum["Selection"]
        cum_fig = go.Figure([
            go.Scatter(x=cum.index, y=cum["Allocation"], name="Allocation",
                       line=dict(color="#2F62F0", width=2)),
            go.Scatter(x=cum.index, y=cum["Selection"], name="Selection",
                       line=dict(color="#E09F3E", width=2)),
            go.Scatter(x=cum.index, y=cum["Total active"], name="Total active",
                       line=dict(color="#1C2233", width=3)),
        ])
        cum_fig.add_hline(y=0, line_dash="dot", line_color="#8A93A8")
        cum_fig.update_layout(title="Cumulative active return vs the 60/40",
                              yaxis_ticksuffix="%")
        st.plotly_chart(style(cum_fig, h=360), use_container_width=True)
        st.caption("Running total of the monthly effects — above zero means that "
                   "decision has added value over the whole window.")

    st.markdown("### ⚡ Where the risk comes from")
    rc = attribution.risk_contribution(att["class_rets"], class_w,
                                       att["port"], att["bench"])
    rr1, rr2 = st.columns([1.4, 1])
    with rr1:
        rc_fig = go.Figure(go.Bar(
            x=[rc["by_class"][c] * 100 for c in cls_order],
            y=cls_order, orientation="h",
            marker_color=[attribution.CLASS_COLORS[c] for c in cls_order],
            text=[f"{rc['by_class'][c] * 100:.0f}%" for c in cls_order],
            textposition="outside"))
        rc_fig.update_layout(title="Share of portfolio risk by asset class",
                             xaxis_ticksuffix="%")
        st.plotly_chart(style(rc_fig, h=320), use_container_width=True)
        st.caption("Each class's weight × how much it moves with the whole "
                   "portfolio, as a share of total variance. Risk is usually far "
                   "more concentrated than money: equities can dominate risk even "
                   "at moderate weights.")
    with rr2:
        r1m, r2m = st.columns(2)
        r1m.metric("Market-driven (beta)", pct(rc["systematic"], 0),
                   help="Share of variance explained by the 60/40 benchmark — "
                        "this is risk you're paid a market premium to hold.")
        r2m.metric("Portfolio-specific", pct(rc["specific"], 0),
                   help="Variance unique to your fund picks and tilts. This is "
                        "where selection skill (or noise) lives.")
        st.metric("R² of the regression", f"{bs['r2']:.2f}",
                  help="How much of the month-to-month movement the 60/40 explains "
                       "on its own.")

    st.caption("⚠️ Fine print: effects use arithmetic (additive) annualisation so "
               "the waterfall sums exactly; the benchmark is USD-based ETF total "
               "returns (ACWI; global agg spliced with US agg before 2018) while "
               "funds are THB-priced — currency effects are inside the numbers.")

# ================================================================== MARKET PULSE
with tabs[7]:
    region_label = st.radio("Region", ["🌍 Global markets", "🇹🇭 Thai market"],
                            horizontal=True, key="pulse_region",
                            label_visibility="collapsed")
    region = "global" if region_label.startswith("🌍") else "th"
    pulse = get_pulse(region)
    trends = get_pulse_trends(region)
    topic_of_term = {t["term"]: t for t in pulse["topics"]}

    TXT = {
        "global": dict(
            intro_title="Search data = the market's mood ring",
            intro=("Google Trends measures what the world is <i>actually</i> worried "
                   "or excited about — often before it shows up in prices. Below: the "
                   "themes dominating financial search interest right now, what they "
                   "mean in plain English, and which part of <i>your</i> plan each one "
                   "touches. Curated by your advisor on {date}."),
            hot="Which themes are running hot? Search interest, last 4 weeks vs each "
                "theme's own 12-month average",
            explore="### 🔍 Explore a theme", theme="Theme",
            interest="Google search interest — “{term}” (12 months, 100 = peak)",
            what="What it is.", why="Why it's everywhere.",
            means="### 🧭 What each theme means for your portfolio",
            news="### 📰 Making headlines right now",
            sources="Sources", see="see",
            fineprint=("Search-interest series come from Google Trends (each scaled "
                       "to its own 12-month peak = 100). This is an advisor-curated "
                       "snapshot — refresh with `scripts/refresh_market_pulse.py` — "
                       "and market commentary, not investment advice."),
        ),
        "th": dict(
            intro_title="ข้อมูลการค้นหา = เครื่องวัดอารมณ์ของตลาด",
            intro=("Google Trends สะท้อนสิ่งที่ผู้คน<i>กังวลหรือตื่นเต้นจริง ๆ</i> — "
                   "บ่อยครั้งก่อนที่จะปรากฏในราคาสินทรัพย์ ด้านล่างคือธีมที่ครองความสนใจ"
                   "การค้นหาด้านการเงินของคนไทยตอนนี้ พร้อมคำอธิบายแบบเข้าใจง่าย "
                   "และความหมายต่อแผนการลงทุนของคุณ · จัดทำโดยที่ปรึกษา ณ วันที่ {date}"),
            hot="ธีมไหนกำลังร้อนแรง? ความสนใจค้นหา 4 สัปดาห์ล่าสุด เทียบค่าเฉลี่ย "
                "12 เดือนของแต่ละธีม",
            explore="### 🔍 เจาะลึกทีละธีม", theme="เลือกธีม",
            interest="ความสนใจค้นหาบน Google — “{term}” (12 เดือน, 100 = จุดสูงสุด)",
            what="เกิดอะไรขึ้น —", why="ทำไมถึงเป็นกระแส —",
            means="### 🧭 แต่ละธีมมีความหมายอย่างไรต่อพอร์ตของคุณ",
            news="### 📰 ข่าวการเงินไทยที่กำลังเป็นกระแส",
            sources="ที่มา", see="ดูแท็บ",
            fineprint=("ดัชนีความสนใจค้นหามาจาก Google Trends (แต่ละธีมปรับสเกลให้"
                       "จุดสูงสุดในรอบ 12 เดือน = 100) เนื้อหาคัดสรรโดยที่ปรึกษา ณ "
                       "วันที่ระบุ — อัปเดตข้อมูลด้วย `scripts/refresh_market_pulse.py` "
                       "— เป็นบทวิเคราะห์เพื่อการศึกษา ไม่ใช่คำแนะนำการลงทุน"),
        ),
    }[region]

    edu_card("📡", TXT["intro_title"],
             TXT["intro"].format(date=f"{pd.Timestamp(pulse['as_of']):%d %B %Y}"))

    momentum = {}
    for term in trends.columns:
        base = float(trends[term].mean())
        if base > 0:
            momentum[term] = float(trends[term].tail(4).mean()) / base - 1
    mom = pd.Series(momentum).sort_values()
    mom_labels = [f"{topic_of_term[t]['icon']} {topic_of_term[t]['title']}"
                  if t in topic_of_term else t for t in mom.index]
    mfig = go.Figure(go.Bar(
        x=mom.values * 100, y=mom_labels, orientation="h",
        marker_color=["#2E8B6B" if v >= 0 else "#8A93A8" for v in mom.values],
        text=[f"{v * 100:+.0f}%" for v in mom.values], textposition="auto"))
    mfig.update_layout(title=TXT["hot"], xaxis_ticksuffix="%")
    st.plotly_chart(style(mfig, h=340), use_container_width=True)

    st.markdown(TXT["explore"])
    sel_title = st.selectbox(
        TXT["theme"], [t["title"] for t in pulse["topics"]],
        format_func=lambda s: f"{next(t['icon'] for t in pulse['topics'] if t['title'] == s)} {s}",
        key=f"pulse_theme_{region}")
    topic = next(t for t in pulse["topics"] if t["title"] == sel_title)
    cch, cdet = st.columns([1.3, 1])
    with cch:
        s = trends[topic["term"]]
        tfig = go.Figure(go.Scatter(x=s.index, y=s, fill="tozeroy",
                                    fillcolor="rgba(47,98,240,0.15)",
                                    line=dict(color="#2F62F0", width=2),
                                    name=topic["term"]))
        tfig.update_layout(title=TXT["interest"].format(term=topic["term"]),
                           yaxis_range=[0, 108])
        st.plotly_chart(style(tfig, h=340), use_container_width=True)
    with cdet:
        edu_card(topic["icon"], topic["title"],
                 f"<b>{TXT['what']}</b> {topic['what']}<br><br>"
                 f"<b>{TXT['why']}</b> {topic['why_now']}")

    st.markdown(TXT["means"])
    for row_start in (0, 3):
        cols = st.columns(3)
        for col, t in zip(cols, pulse["topics"][row_start:row_start + 3]):
            with col:
                edu_card(t["icon"], t["title"],
                         f"{t['for_you']}<br><br>"
                         f"<i>{TXT['sources']}: {', '.join(t['sources'])} · "
                         f"{TXT['see']} {t['related_tab']}</i>")

    if pulse.get("news"):
        st.markdown(TXT["news"])
        news_cols = st.columns(2)
        for i, item in enumerate(pulse["news"]):
            with news_cols[i % 2]:
                st.markdown(
    ...
    st.caption(f"📡 {pulse['intro_note']} {TXT['fineprint']}")
...
with tabs[8]:
    ...
    with st.expander("📖 Glossary — every term on this dashboard"):
        st.dataframe(pd.DataFrame([
            ...
            ["Active return", "Your return minus the 60/40 benchmark's return — the part your decisions are responsible for."],
            ["Beta", "How strongly the portfolio moves with the benchmark. Beta 0.7 = when the 60/40 moves 1%, you typically move 0.7%."],
            ["Alpha", "Return left over after accounting for beta — the part not explained by simply riding the market."],
            ["Allocation effect", "Value added (or lost) by holding different amounts of each asset class than the 60/40 does."],
            ["Selection effect", "Value added (or lost) by the specific funds chosen, versus their own asset class index."],
            ["Tracking error", "Typical yearly gap between the portfolio and the benchmark — how differently it behaves."],
